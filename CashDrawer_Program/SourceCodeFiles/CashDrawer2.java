...
public String getChangeBAck(int input) {
...
public void getItemCost() {
	...
	costAmount((int)Math.round(Double.parseDouble(scanner.nextLine())*100));
}
//gets the payment from the user
public void getCustomerPayment() {
	//converts the scanner to a double then times it by 100 then math rounds it then converts it into an int
	paymentAmount((int)Math.round(Double.parseDouble(scanner.nextLine())*100));
}
...
public void getChangeBack() {
	...
	//prints out the change back
	System.out.println("Change Back:"+getChangeBAck(exchange[2]));
}
...
public void program() {
	System.out.println("Cost Of Items:[example format 1.03]");
	getItemCost();
	System.out.println("Customer Payment Amount:[example format 1.03]");
	getCustomerPayment();
	getChangeBack();
...
public static void main(String[] args) {
	CashDrawer2 object = new CashDrawer2();
	object.program();
}
}
